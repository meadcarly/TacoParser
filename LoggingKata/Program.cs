...
namespace LoggingKata
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (lines.Length == 0)
            {
                logger.LogError($"Sorry, there were zero lines in the file");
            }

            if (lines.Length == 1)
            {
                logger.LogWarning("You only have one line in the file.");
            }
            ...
            for (int i = 0; i < locations.Length; i++)
            {
                ...
                for (int j = i + 1; j < locations.Length; j++)
                {
                    ...
                    var distanceAToB = corA.GetDistanceTo(corB);
                    if (distanceAToB > furthestDistance)
                    {
                        furthestDistance = distanceAToB;
                        startPoint = locA;
                        endPoint = locB;
                    }
                }
            ...
            logger.LogInfo($" The furthest locations apart are:\n{startPoint.Name} {startPoint.Location.Latitude}, {startPoint.Location.Longitude} and {endPoint.Name} at {endPoint.Location.Latitude}, {endPoint.Location.Longitude}");
            logger.LogInfo($"The distance between the two is {furthestDistance} meters.");
